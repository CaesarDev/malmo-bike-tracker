...
<div class="px-6 py-4 w-full max-w-md rounded overflow-hidden shadow-lg bg-white">
    <div class="">
        ...
        @if($stationStatus['availability']['bikes'] > 5)
            <span class="inline-block bg-green-200 rounded-full px-3 py-1 text-sm font-semibold text-green-700 mr-2 mb-2">Plenty bikes</span>
        @else
            <span class="inline-block bg-red-200 rounded-full px-3 py-1 text-sm font-semibold text-red-700 mr-2 mb-2">Few bikes</span>
        @endif
        ...
        <p class="text-gray-700 text-base">
            <span class="font-semibold">Total Docks:</span> {{ $stationStatus['availability']['bikes'] + $stationStatus['availability']['slots'] }}
        </p>
        <p class="text-gray-700 text-base">
            <span class="font-semibold">Location:</span> {{ $stationInfo['location']['lat'] }}, {{ $stationInfo['location']['lon'] }}
        </p>
    ...
    <div class="pt-4 pb-2 bg-gray-50">
        ...
        <x-pill color="{{ $stationStatus['statusDetail']['operationalStatus'] == 'Operational' ? 'green' : 'red' }}">
    ...
    <div>
        ...
        <ul>
            @foreach($nearbyStations as $nearbyStation)
                <li>
                    <a href="/station/{{ $nearbyStation['id'] }}" class="underline mb-2">
